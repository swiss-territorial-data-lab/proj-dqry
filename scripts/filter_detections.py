...
if __name__ == "__main__":
    ...
    def get_mean_slope_for_detection(detection_geometry, slope_raster):
        ...
        # Calculate mean slope for each detection
    detections['mean_slope'] = detections.geometry.apply(lambda geom: get_mean_slope_for_detection(geom,slope_raster))

        # Filter detections based on slope threshold
    detections = detections[detections['mean_slope'] < DEGREE]  # Apply the slope threshold
        # Log how many detections were removed by slope threshold
    te_slope = len(detections)
    logger.info(f"{total - te_slope} detections were removed by slope threshold: {DEGREE} degrees")
