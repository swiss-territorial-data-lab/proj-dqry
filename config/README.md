...
Configurations files are used to set the variables parameters. They must be adapted if required. In **config** folder config files relative to `proj-dqry` and `object-detector` are present, one for each workflow type:

1. Training and evaluation: `config-trne.yaml` 
2. Predictions: `config-prd.yaml` 
3. Detection monitoring: `config-dm.yaml` 

A config file dedicated to set the parameters of the detectron2 algoorithm is also provided: `detectron2_config_dqry.yaml`
...
Specify as well the zoom level _z_. The zoom level will act on the tiles size (and tiles number) and on the pixel resolution. We advice to use zomme level between 16 (1.6 m/px) and 17 (0.8 m/px).
The **srs** key provides the working geographical frame to ensure all the input data are compatible together.

Then, by running `generate_tilesets.py` the images will be donwloaded from a _WMTS_ according the the tiles characteristcs defined previously. A _XYZ_ connector is used to access _SWISSIMAGE_ for a given year. Be carreful to set the desired year in the url:
...
The algorithm is trained with script `train_model.py` calling detectron2 algorithm. For object detection, instance segmentation is used `COCO-InstanceSegmentation/mask_rcnn_R_50_FPN_1x.yaml`. The model characteristics can be tuned by modifying parameters in `detectron2_config_dqry.yaml`. 
Especially pay attention to the **MAX_ITER** and **STEPS** values (Last **STEPS** < **MAX_ITER**). Choose a number of iteration to ensure robust model training. The model training evolution can be monitor with `tensorboard`. The library `PyTorch` must be installed (installed and activated the virtual environment). The validation loss curve can be visualized and the optimum epoch of the model, _i.e._ the epoch minimizing the validation loss curve can be identified. This epoch corresponds to the optimum and must set as input trained model for `make_predictions.py` (model_weights: pth_file:./logs/[chosen model].pth). The optimum is dependant of the zoom level and the number of tiles involved. The file `model_final.pth` corresponds to the last iteration recorded during the training procedure. Trained model at different epochs is saved in folder **logs**.
 

    tensorboard --logdir <logs folder path>/logs
Open the following link with a web browser: `http://localhost:6006`

Predictions come with a confidency score [0 to 1]. Predictions with low score can be discarded by setting a threshold value for `make_predictions.py` script **score_thd**. 
...
Specify as well the zoom level _z_. The zoom level will act on the tiles size (and tiles number) and on the pixel resolution. We advice to use zomme level between 16 (1.6 m/px) and 17 (0.8 m/px). The zoom level should be the same than used model has been trained.
The **srs** key provides the working geographical frame to ensure all the input data are compatible together.
...
A **debug mode** can be activated in order to run the script on a sample of images to perform some test for instance. The number of images sampled is hard-coded in the script. Inference prediction are performed (no Ground Truth data provided). One dataset will be defined (_oth_).
