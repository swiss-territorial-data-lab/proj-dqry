...
* Shapefiles
	...
	- `swissimage_footprints_shape_year_per_year`: original _SWISSIMAGE_ footprints and processed/corrected polygons border shapefiles for every acquisition year (1946 to 2021). Shapefiles located in the forlder **swissimage_footprints_border** must be used as AOI to perform inference prediction except for years 2005, 2006, 2007 and 2021 for which shapefiles required correction (see the explanation here: https://tech.stdl.ch/PROJ-DQRY/) and can be found in the folders **swissimage_footprints_correction**. For year 2020, the file _tiles.shp_ located in the folder **swissimage_tiles_correction** has to be used as input of the `object-detector` script `generate_tilesets.py`. There is currently issue to download some images from 2020 dataset and the missing tiles have been removed.
